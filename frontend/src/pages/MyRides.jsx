...
const MyRides = () => {
  ...
  const fetchRides = async (filtersToApply = {}, sortParam = { field: "", order: "" }) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (sortParam.field) {
        ridesData = ridesData.sort((a, b) => {
          const fieldA = sortParam.field === "fare" ? a.finalFare || 0 : new Date(a.departureTime);
          const fieldB = sortParam.field === "fare" ? b.finalFare || 0 : new Date(b.departureTime);
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 p-6">
      ...
      {loading && <p className="text-center text-gray-600 dark:text-gray-400">Loading rides...</p>}
           ...
           <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
             ...
             {ride.rideType === "scheduled" && (
               <div className="flex justify-between">
                 ...
                 <span className="capitalize">{ride.scheduledType || "N/A"}</span>
               </div>
             )}
         
             <div className="flex justify-between">
               <span className="font-medium">Departure:</span>
               <span>{ride.departureTime ? new Date(ride.departureTime).toLocaleString() : "N/A"}</span>
             </div>
         
             {ride.estimatedArrivalTime && (
               <div className="flex justify-between">
                 <span className="font-medium">ETA:</span>
                 <span>{new Date(ride.estimatedArrivalTime).toLocaleString()}</span>
               </div>
             )}
             ...
             <div className="flex justify-between">
               ...
               <span>{ride.seatsBooked} / {ride.totalSeats}</span>
             </div>
